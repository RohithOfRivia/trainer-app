<script lang="ts">
    ...
    let workout = $state({
        ...
        notes: "",
        exercises: [
            { id: 1, name: "", sets: 3, reps: 10, weight: 0, completed: false },
        ] as Exercise[],
    });
    ...
    function toggleComplete(exercise: Exercise) {
...
<div
    class="w-full max-w-[580px] mx-auto px-4 py-6 sm:py-10 flex flex-col gap-5 font-sans"
>
                ...
                {:else}
                    ...
                    <div class="flex items-center gap-1.5 text-xs mt-1">
                        ...
                        >
                            ...
                            <Icon
                                name="edit"
                                className="text-[12px] text-base-content/50 group-hover:text-base-content"
                            />
                            <input
                                type="date"
    ...
    <div class="flex flex-col gap-3">
        {#each workout.exercises as exercise, index (exercise.id)}
            ...
            <div
                ...
                class="bg-base-content/5 border border-base-content/10 rounded-2xl overflow-visible transition-all {exercise.completed
            ...
            >
                ...
                >
                    <span
                        class="shrink-0 w-6 h-6 rounded-md bg-base-content/10 text-base-content/60 text-xs font-semibold flex items-center justify-center transition-colors {exercise.completed
                            ? 'bg-primary text-primary-content'
                            : ''}"
                    >
                        {#if exercise.completed}<Icon
                                name="check"
                                className="text-sm"
                            />{:else}{index + 1}{/if}
                    </span>
                    ...
                    >
                        <div class="relative">
                            <input
                                ...
                                onkeydown={(e) =>
                                    handleKeydown(e, "sets", exercise.id)}
                            />
                            {#if activeAutocomplete === exercise.id && autocompleteOptions(exercise.name).length > 0}
                                ...
                                >
                                    {#each autocompleteOptions(exercise.name) as s}
                                        <li>
                                            <button
                                                class="w-full text-left px-4 py-2 text-sm text-base-content/80 hover:bg-primary/10 hover:text-base-content flex items-center gap-2 transition-colors"
                                                onmousedown={() => {
                                                    exercise.name = s;
                                                    activeAutocomplete = null;
                                                    focusNext(
                        ...
                        >
                            ...
                            {@render Stepper(
                                exercise,
                                "reps","Reps",0,1,"weight",
                            )}
                            {@render Stepper(
                                exercise,
                                "weight","Weight",0,5,null,
                            )}
                        </div>
                        ...
                        <div class="flex justify-end items-center pt-2">
                            ...
                            {#if workout.exercises.length > 1}
                                <ActionButton
                                    icon="delete"
                                    color="btn-error"
                                    className="btn-sm btn-circle hover:text-error hover:bg-error/10 border-none opacity-70 hover:opacity-100 transition-opacity"
